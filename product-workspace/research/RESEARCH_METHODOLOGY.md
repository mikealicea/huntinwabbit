...
- **Decision-first research.** Product and engineering research exists to drive a decision, so the index leads with an
  **Executive Summary that distinguishes findings, recommendations, and confirmed decisions** and an **Action Plan** with checkboxes (standard
  Part 6).
- **Actionable companion files** are common here (standard Part 2): `*.DELIVERABLES.md`,
  `*.QUICK_START.md`, `*.summary.md`, `*.sources-index.md`. Add them when a project needs an execution
  roadmap or a centralized source registry; skip them otherwise.
- **Claim → Evidence → Why it matters → Source.** Keep the source, but always include the product or engineering
  implication beat (standard Part 5).

## See Also

- [[RESEARCH.METHODOLOGY.AGENTS]] — canonical standard (read this)
- [[examples-research-README]] — copyable project scaffold
- [[RESEARCH_QUICK_REFERENCE]] · [[WORKSPACE_STYLE_GUIDE]] — local quick aids
